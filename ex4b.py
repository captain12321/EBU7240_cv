...
class Classifier(object):
    ...
    def get_data(self):
        ...
        test = np.reshape(data[b'data'][2000:4000,:], (2000, 3, 32 * 32))
        labels = np.reshape(data[b'labels'][2000:4000], (2000, 1))
        for i in range(len(labels)):
            if labels[i] != 0:
                labels[i] = 1  # 等于1说明不是飞机
        fileNames = np.reshape(data[b'filenames'][2000:4000], (2000, 1))
    ...
    def classification(self, train_feat, test_feat,test):
        clf = LinearSVC()
        ...
        for i in plane:
            ...
            cv2.imwrite('../results/HOG_plane{0}.jpg'.format(count),img)
            cv2.waitKey()
        count=0
        for i in not_plane:
            count+=1
            ...
            cv2.imwrite('../results/HOG_not_plane{0}.jpg'.format(count), img)
            cv2.waitKey()
    def run(self):
        TrainData, TestData,test = self.get_data()
        train_feat, test_feat = self.get_feat(TrainData, TestData)
        self.classification(train_feat, test_feat,test)
